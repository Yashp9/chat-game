import React from "react";

const NoChatSelected = () => {
  return (
    <div className="w-full flex flex-1 flex-col items-center justify-center p-16 bg-base-100/50 relative overflow-hidden">
      {/* Background Elements */}
      <div 
        className="absolute w-[400px] h-[400px] bg-gradient-to-tr from-emerald-200 to-teal-300 opacity-20 rounded-full animate-pulse blur-3xl z-0" 
        style={{ top: '30%', left: '50%', transform: 'translate(-50%, -50%)' }} 
      />

      {/* Floating leaves */}
      <div className="absolute top-20 left-1/4 w-8 h-8 animate-float-slow opacity-70">
        <div className="w-full h-full bg-emerald-400 rounded-tl-3xl rounded-br-3xl rotate-45" />
      </div>
      <div className="absolute top-40 right-1/3 w-6 h-6 animate-float-medium opacity-60" style={{ animationDelay: "1.5s" }}>
        <div className="w-full h-full bg-teal-300 rounded-tl-3xl rounded-br-3xl rotate-12" />
      </div>
      <div className="absolute bottom-40 right-1/4 w-10 h-10 animate-float-slow opacity-50" style={{ animationDelay: "2.2s" }}>
        <div className="w-full h-full bg-emerald-300 rounded-tl-3xl rounded-br-3xl rotate-30" />
      </div>

      {/* Content Card */}
      <div className="max-w-md w-full border border-emerald-100 shadow-md bg-white/60 backdrop-blur-sm rounded-xl z-10 p-6 text-center space-y-6">
        {/* Icon with ripple */}
        <div className="flex justify-center mb-4">
          <div className="relative">
            <div className="absolute inset-0 w-24 h-24 rounded-full animate-ripple border-2 border-teal-300/50"></div>
            <div className="absolute inset-0 w-24 h-24 rounded-full animate-ripple border-2 border-emerald-400/30" style={{ animationDelay: "1s" }}></div>
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-teal-100 to-emerald-200 flex items-center justify-center shadow-md relative overflow-hidden" style={{ top: "8px", left: "9px" }}>
              <div className="absolute inset-0 bg-white/40 rounded-full animate-pulse-gentle"
              ></div>
              {/* Updated Image Position */}
              <img 
                src="data:image/png;base64,"
                alt="Monk Icon"
                className="z-10 absolute"
                
              />
            </div>
          </div>
        </div>

        {/* Text */}
        <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-600">Welcome to Monk-Chat!</h2>

      </div>

      {/* Custom Animations */}
      <style >{`
        @keyframes float-slow {
          0%, 100% { transform: translateY(0); }
          50% { transform: translateY(-20px); }
        }

        @keyframes float-medium {
          0%, 100% { transform: translateY(0); }
          50% { transform: translateY(-30px); }
        }

        @keyframes ripple {
          0% { transform: scale(0.8); opacity: 1; }
          100% { transform: scale(2.5); opacity: 0; }
        }

        @keyframes pulse-gentle {
          0%, 100% { opacity: 0.4; }
          50% { opacity: 0.8; }
        }

        .animate-float-slow {
          animation: float-slow 6s ease-in-out infinite;
        }

        .animate-float-medium {
          animation: float-medium 5s ease-in-out infinite;
        }

        .animate-ripple {
          animation: ripple 4s infinite ease-out;
        }

        .animate-pulse-gentle {
          animation: pulse-gentle 3s infinite ease-in-out;
        }
      `}</style>
    </div>
  );
};

export default NoChatSelected;


// import { MessageSquare } from "lucide-react";
// import { useThemeStore } from "../store/useThemeStore";

// const NoChatSelected = () => {
//   const {theme} = useThemeStore();
//   return (
//     <div className="w-full flex flex-1 flex-col items-center justify-center p-16 bg-base-100/50">
//       <div className="max-w-md text-center space-y-6">
//         {/* Icon Display */}
//         <div className="flex justify-center gap-4 mb-4">
//           <div className="relative">
//             <div
//               className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center
//              justify-center animate-bounce"
//             >
//                {theme === "dark" ? <img src="data:image/png;base64," alt="external-monk-buddhism-icongeek26-outline-colour-icongeek26"/>:
//               <img src="data:image/png;base64," alt="external-monk-buddhism-icongeek26-outline-colour-icongeek26"/>}

//             </div>
//           </div>
//         </div>

//         {/* Welcome Text */}
//         <h2 className="text-2xl font-bold">Welcome to Monk-Chat!</h2>
//         <p className="text-base-content/60">
//           Select a conversation from the sidebar to start chatting
//         </p>
//       </div>
//     </div>
//   );
// };

// export default NoChatSelected;
